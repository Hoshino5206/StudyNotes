...
## (2) redis集群配置（低版本3.2左右）

1.安装Redis------https://github.com/tporadowski/redis/releases（5.0.10）

2.安装Ruby------https://rubyinstaller.org/downloads/（3.0.3-1）

3.下载ruby环境下Redis的驱动（注意考虑兼容性）------https://rubygems.org/gems/redis/versions/4.5.0（4.5.0）

4.安装驱动

```
gem install --local 文件完整路径
```

5.下载Redis官方提供的创建Redis集群的ruby脚本文件redis-trib.rb------https://raw.githubusercontent.com/MSOpenTech/redis/3.0/src/redis-trib.rb（也可以使用redis源码目录下src的redis-trib.rb）
...
## (3) redis集群配置（高版本5.0左右）

1.安装Redis------https://github.com/tporadowski/redis/releases（5.0.10）

2.创建集群（低版本需要安装ruby，高版本不需要，该操作会在nodes文件夹里自动创建集群配置信息，前三个主，后四个从）

```
redis-cli --cluster create 127.0.0.1:6380 127.0.0.1:6381 127.0.0.1:6382
```

![redis集群](C:\Users\Akino\Desktop\StudyNotes\Image\redis集群.jpg)
