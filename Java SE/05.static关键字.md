...
# D.同步锁方法及变量被static修饰原则（多线程）

## 1.实现Runnable接口:

线程对象此时对于创建的多个线程来说是共享的（也就是说多个线程操作的是同一个对象），因此方法不需要用static修饰，变量也不需要用static修饰

## 2.继承Thread类:

总结：单一原则，只要变量或方法需要被共享的，要考虑对象是否单例，如果有多次被创建的可能，则变量或方法需要做共享处理（被static修饰）

知识点补充：在类加载时，如果用static修饰的变量或方法在类加载是时候，就已经加载到方法区，而成员变量只有实例化时才被加载，实例多少个，成员变量也拥有多少个，而静态变量从始至终都只有一个